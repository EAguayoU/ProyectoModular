...
import com.imbitbox.recolectora.config.ExperimentalSignatureApi
...
class clSignaturePad(context: Context, attrs: AttributeSet?) : View(context, attrs) {

    private val originalEvents = mutableListOf<clEvent>()
    private var iter: MutableIterator<clEvent> = mutableListOf<clEvent>().iterator()

    // View state
    private val points = mutableListOf<clTimedPoint>()
    ...
    private val mSvgBuilder = clSvgBuilder()

    // Cache
    private val mPointsCache: MutableList<clTimedPoint?> = ArrayList()

    // Configurable parameters
    private var mMinWidth = 0
    private var mMaxWidth = 0
    private var mVelocityFilterWeight = 0f
    private var mSignedListener: iSignedListener? = null
    ...
    override fun onRestoreInstanceState(state: Parcelable) {
        ...
        if (state is Bundle) {
            ...
            val events: Array<clEvent> =
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    state.getParcelableArray("events", clEvent::class.java)
                } else {
                    @Suppress("DEPRECATION")
                    state.getParcelableArray("events")?.map { it as clEvent }?.toTypedArray()
                } ?: emptyArray()
    ...
    private fun addEvent(event: clEvent) {
    ...
    private fun current(event: clEvent) {
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        return when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                val downEvent = clEvent(System.currentTimeMillis(), event.action, event.x, event.y)
                addEvent(downEvent)
                invalidate()
                true
            }
            MotionEvent.ACTION_MOVE -> {
                val moveEvent = clEvent(System.currentTimeMillis(), event.action, event.x, event.y)
                addEvent(moveEvent)
                invalidate()
                true
            }
            MotionEvent.ACTION_UP -> {
                val upEvent = clEvent(System.currentTimeMillis(), event.action, event.x, event.y)
                addEvent(upEvent)
                invalidate()
                true
            }
    ...
    fun setOnSignedListener(listener: iSignedListener?) {
    ...
    fun getSignature(): clSignature {
        return clSignature(BuildConfig.VERSION_CODE, originalEvents)
    }

    @ExperimentalSignatureApi
    fun setSignature(signature: clSignature) {
        clear()
    ...
    private fun getNewTimedPoint(x: Float, y: Float, timestamp: Long): clTimedPoint {
        val cacheSize = mPointsCache.size
        val timedPoint: clTimedPoint? = if (cacheSize == 0) {
    ...
    private fun recyclePoint(point: clTimedPoint?) {
        mPointsCache.add(point)
    }

    @SuppressWarnings("MagicNumber")
    private fun addTimedPoint(timedPoint: clTimedPoint, timestamp: Long) {
        points.add(timedPoint)
        val pointsCount = points.size
        if (pointsCount > 3) {
            var tmp: clControlTimedPoints =
            ...
            val c3 = tmp.c1
            recyclePoint(tmp.c2)
            val curve = clBezier(points[1], c2, c3, points[2])
    ...
    @SuppressWarnings("MagicNumber")
    private fun addBezier(curve: clBezier, startWidth: Float, endWidth: Float) {
